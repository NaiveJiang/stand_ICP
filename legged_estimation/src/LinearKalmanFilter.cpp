  ...
  , topicUpdated_(false)
{
  ...
  vs_.setZero();
  a_.setZero();
  a_.block(0, 0, 3, 3) = Eigen::Matrix<scalar_t, 3, 3>::Identity();   //pos
  a_.block(3, 3, 3, 3) = Eigen::Matrix<scalar_t, 3, 3>::Identity();   //vel
  a_.block(6, 6, 24, 24) = Eigen::Matrix<scalar_t, 24, 24>::Identity(); //pos contact
  b_.setZero();
  ...
  //base和足端的相对位置  pos_b - pos_c_i  i=0,1,...,8
  c_.block(0, 0, 3, 6) = c1;
  c_.block(3, 0, 3, 6) = c1;
  c_.block(6, 0, 3, 6) = c1;
  c_.block(9, 0, 3, 6) = c1;
  ...
  c_.block(0, 6, 24, 24) = -Eigen::Matrix<scalar_t, 24, 24>::Identity();
...
vector_t KalmanFilterEstimate::update(const ros::Time& time, const ros::Duration& period)
{
  ...
  q_.block(6, 6, 24, 24) = dt * Eigen::Matrix<scalar_t, 24, 24>::Identity();
  ...
  Eigen::Matrix<scalar_t, 30, 30> q = Eigen::Matrix<scalar_t, 30, 30>::Identity();
  q.block(0, 0, 3, 3) = q_.block(0, 0, 3, 3) * imuProcessNoisePosition_;
  q.block(3, 3, 3, 3) = q_.block(3, 3, 3, 3) * imuProcessNoiseVelocity_;
  q.block(6, 6, 24, 24) = q_.block(6, 6, 24, 24) * footProcessNoisePosition_;

  // the covariance of the observation noise
  Eigen::Matrix<scalar_t, 56, 56> r = Eigen::Matrix<scalar_t, 56, 56>::Identity();
  r.block(0, 0, 24, 24) = r_.block(0, 0, 24, 24) * footSensorNoisePosition_;
  r.block(24, 24, 24, 24) = r_.block(24, 24, 24, 24) * footSensorNoiseVelocity_;
  const int fn = info_.numThreeDofContacts; //8
  r.block(48, 48, fn, fn) = r_.block(48, 48, fn, fn) * footHeightSensorNoise_;
  ...
  Eigen::Matrix<scalar_t, 56, 1> y;
  y << ps_, vs_, feetHeights_;
  xHat_ = a_ * xHat_ + b_ * accel;  
  Eigen::Matrix<scalar_t, 30, 30> at = a_.transpose();
  Eigen::Matrix<scalar_t, 30, 30> pm = a_ * p_ * at + q;  //Pk+1
  Eigen::Matrix<scalar_t, 30, 56> cT = c_.transpose();
  Eigen::Matrix<scalar_t, 56, 1> yModel = c_ * xHat_;
  Eigen::Matrix<scalar_t, 56, 1> ey = y - yModel;        
  Eigen::Matrix<scalar_t, 56, 56> s = c_ * pm * cT + r;  

  Eigen::Matrix<scalar_t, 56, 1> sEy = s.lu().solve(ey);  
  xHat_ += pm * cT * sEy;                                 

  Eigen::Matrix<scalar_t, 56, 30> sC = s.lu().solve(c_);
  p_ = (Eigen::Matrix<scalar_t, 30, 30>::Identity() - pm * cT * sC) *
       pm;  

  Eigen::Matrix<scalar_t, 30, 30> pt = p_.transpose();
  p_ = (p_ + pt) / 2.0; //保证对称性

  if (p_.block(0, 0, 2, 2).determinant() > 0.000001)
  {
    p_.block(0, 2, 2, 28).setZero();
    p_.block(2, 0, 28, 2).setZero();
    p_.block(0, 0, 2, 2) /= 10.;
  }
  ...
  updateLinear(xHat_.segment<3>(0), xHat_.segment<3>(3));   //更新位置和线速度

  // std::cout << "***************xHat***************" << std::endl;
  // std::cout << xHat_ << std::endl;
...
void KalmanFilterEstimate::resetEstimationCallback(const std_msgs::Float32::ConstPtr& msg)
{
  ...
  a_.block(6, 6, 24, 24) = Eigen::Matrix<scalar_t, 24, 24>::Identity();
  b_.setZero();
