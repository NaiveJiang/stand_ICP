...
namespace legged {
  FootPlacementVisualization::FootPlacementVisualization(const SwingTrajectoryPlanner &swingTrajectoryPlanner, size_t numFoot,
                                                         ros::NodeHandle &nh, scalar_t maxUpdateFrequency)
      : swingTrajectoryPlanner_(swingTrajectoryPlanner),
        numFoot_(numFoot),
  ...
  void FootPlacementVisualization::update(const SystemObservation &observation)
  {
    if (observation.time - lastTime_ > minPublishTimeDifference_)
    {
      ...
      {
        auto middleTimes = swingTrajectoryPlanner_.getMiddleTimes(leg);

        int kStart = 0;
        for (int k = 0; k < middleTimes.size(); ++k)
        {
          const auto projection = swingTrajectoryPlanner_.getProjection(leg, middleTimes[k]);
          ...
          // Convex Region
          const auto convexRegion = swingTrajectoryPlanner_.getConvexPolygon(leg, middleTimes[k]);
          ...
          // Nominal Footholds
          // const auto nominal = swingTrajectoryPlanner_.getNominalFootholds(leg, middleTimes[k]);
